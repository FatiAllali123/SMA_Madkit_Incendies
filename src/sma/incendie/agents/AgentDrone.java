...
/**
 * AgentDrone — G1 | Rôle : Drone_Surveillance
 * Patrouille au-dessus de la forêt. Confirme visuellement les feux
 * détectés par les capteurs et envoie des alertes au coordinateur.
 */
public class AgentDrone extends Agent {
    ...
    protected void live() {
        ...
        while (actif && batterie > 10) {
            // Lire les messages (alertes des capteurs)
            Message msg;
            while ((msg = nextMessage()) != null) {
                if (msg instanceof AlerteMessage) {
                    ...
                    if (a.getIndiceDanger() >= AGRConstants.SEUIL_ORANGE) {
                        ...
                        getLogger().warning("[" + id + "] Feu confirmé visuellement ! Danger~" + dangerEstime);
                    }
            ...
            // Action selon état
            if (incendieVu) {
                // Envoyer confirmation visuelle au coordinateur
